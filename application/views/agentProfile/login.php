			...
			<div class="row h-100">
				<div class="col-lg-5 col-12">
					<div id="auth-left">
						...
						<h1 class="auth-title">تسجيل الدخول الوكيل</h1>
						<p class="auth-subtitle mb-5">الرجاء ادخال البريد الالكتروني وكلمة المرور بشكل صحيح</p>

						<form id="do_login" method="POST" action="<?=site_url()?>agentProfile/do_login">
							...
							<button class="btn btn-primary btn-block btn-lg shadow-lg mt-5 btn-submit" type="submit">
								<span class="spinner-border spinner-border-sm d-none mt-2" role="status" aria-hidden="true"></span>
						...
						$.ajax({
							...
							success: function(res){ 
								...
								if(res.status == 1){
									runToastify(res.res);
									setTimeout(function(){
										window.location.href = "<?= site_url()?>agentProfile";
									}, 2000);
								} else {
							...
							error: function(){ 
								Swal.fire("خطأ !!", "bad request ..!", "error");
							}
						});
						$(".btn-submit .spinner-border").toggleClass("d-none");
