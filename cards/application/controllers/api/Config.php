...
class Config extends RestController{
	...
	// get most order card
	public function most_order_card_get(){
		$res = $this->api_config_model->get_most_order_card();
	...
	public function category_get($category_id, $language){
		...
		if(!empty($res)){
			...
			for ($i = 0; $i < count($res) ; $i++) {
				$response = $res[$i];
				$row =  $this->api_config_model->get_category($response->category_id);
				if(!empty($row)){
				    $no_sub_category = 1;
				} else {
				    $no_sub_category = 0;
				...
				$data = [
					...
					'no_sub_category'   => (int) $no_sub_category
				];
				$all_data[] = $data;
			}
		    $this->response( $all_data, 200);
        /*    $this->response([
				"no_sub_categeory" => $no_sub_categeory,
				"data" => $all_data
			], 200);
			*/
			...
			if(!empty($res)){
				...
				for ($i = 0; $i < count($res) ; $i++) {
					...
					$data = [
						...
						'card_min_amount'	=> $response->card_min_amount,
